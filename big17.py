#Big 17 Balance checker 13 Working 15/05/2021 Mizogg&Chad mizogg.co.uk Random
import random
import bit
from bit import *
...
def spawn():  
    ...
    #API Request Weblist
        ...
        blocdash = requests.get(" https://api.blockcypher.com/v1/dash/main/addrs/"+ str (dashcaddr)) #DASH compressed address API
        resdash = blocdash.json()
        balancedash = dict(resdash)['balance']
        blocdash1 = requests.get(" https://api.blockcypher.com/v1/dash/main/addrs/"+ str (dashuaddr)) #DASH Uncompressed  API
        resdash1 = blocdash1.json()
        balancedash1 = dict(resdash1)['balance']        
        ...
        #bloczen = requests.get("https://explorer.zen-solutions.io/address/" + zencaddr) #Zen compressed address API
        #reszen = bloczen.json()
        #balancezen = dict(reszen)["totalRecv"]
        #bloczen1 = requests.get("https://explorer.zen-solutions.io/address/" + zenuaddr) #Zen Uncompressed address API
        #reszen1 = bloczen1.json()
        #balancezen1 = dict(reszen1)["totalRecv"]
    # Running Display Output
        ...
        print (colour_cyan + 'DASH Compressed Address             ' + '  : '  + colour_red + str (dashcaddr) + '             : ' + colour_reset + 'Balance = ' +  str(balancedash)) #DASH  Compressed address display
        print (colour_cyan + 'DASH Uncompressed Address           ' + '  : '  + colour_red + str (dashuaddr) + '             : ' + colour_reset + 'Balance = ' +  str(balancedash1)) #DASH  Uncompressed address display
        ...
        print (colour_cyan + 'ZEN Compressed Address              ' + '  : '  + colour_red + str (zencaddr) + '            : ' + colour_reset + 'Total Received = ' +  'COMING SOON ') #ZEN  Compressed address display
        print (colour_cyan + 'ZEN Uncompressed Address            ' + '  : '  + colour_red + str (zenuaddr) + '            : ' + colour_reset + 'Total Received = ' +  'COMING SOON ') #ZEN  Uncompressed address display
    ...
    #Results Winner TEXT
        ...
        if float(balanceDoge1) > float(ammount):
            ...
            f.write('\n==================================')
            f.close()
        if int(balancedash) > 0:
            print("Matching Key ==== Dashcoin Compressed address Found!!!\n PrivateKey: " + key.to_hex()) #Dash coin Compressed address winner
            f=open(u"winner.txt","a")
            f.write('\nPrivateKey (hex): ' + key.to_hex())
            f.write('\nDash coin Compressed address: ' + str (dashcaddr))
            f.write('\n==================================')
            f.close()
        if int(balancedash1) > 0:
            print("Matching Key ==== Dashcoin  UnCompressed address Found!!!\n PrivateKey: " + key.to_hex()) #Dash coin  UnCompressed address winner
            f=open(u"winner.txt","a")
            f.write('\nPrivateKey (hex): ' + key.to_hex())
            f.write('\nDashcoin  UnCompressed address: ' + str (dashuaddr))
            f.write('\n==================================')
            f.close()        
        if int(balanceszc) > 0:
        ...
        if int(balanceszc1) > 0:
            ...
            f.close()
        #if int(balancezen) > 0:
            #print("Matching Key ==== Zen Compressed address Found!!!\n PrivateKey: " + key.to_hex()) #Zen Compressed address winner
            #f=open(u"winner.txt","a")
            #f.write('\nPrivateKey (hex): ' + key.to_hex())
            #f.write('\n Zcash Address: ' + zencaddr)
            #f.write('\n==================================')
            #f.close()
        #if int(balancezen1) > 0:
            #print("Matching Key ==== Zen UnCompressed address  Found!!!\n PrivateKey: " + key.to_hex()) #Zen UnCompressed address winner
            #f=open(u"winner.txt","a")
            #f.write('\nPrivateKey (hex): ' + key.to_hex())
            #f.write('\n Zen Address: ' + zenuaddr)
            #f.write('\n==================================')
            #f.close()
