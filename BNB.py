...
while True:
    ...
    count+=1
    blocs = requests.get("https://api.bscscan.com/api?module=account&action=balance&address=" + bnbadd + "&apikey=YourApiKeyToken") #BNB Binance Smart Chain API Must create account to be better API
    ress = blocs.json()
    balance = dict(ress)["result"]
    print ("\n " + colour_cyan + "BNB Binance Smart Chain Random Scan : " + str (count) + colour_red + " : ---Good--Luck--Happy--Hunting--Mizogg.co.uk&Chad---" + colour_cyan + "With Balance Checker" + colour_reset) # Running Display Output
